...
> Quill 编辑器（quill.js / quill.snow.css）已内置在 `static/quill/` 目录中，无需额外下载。
...
│   │   └── style.css               # 极客暗色主题样式
│   ├── quill/
│   │   ├── quill.js                # Quill 编辑器核心（已内置）
│   │   └── quill.snow.css          # Quill 主题样式（已内置）
│   └── uploads/
│       ├── logo/                   # Logo 文件
